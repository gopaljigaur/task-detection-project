            ...
            for image_file in os.listdir(os.path.join(path, task_dir)):
                if image_file.endswith(".png"):
                    ...
                    pil_image = Image.open(os.path.join(
                        path, task_dir, image_file)).convert('RGB')
                    ...
                    fig.suptitle(
                        'Select points on the image (LClick). Next Image (RClick)', fontsize=16)
                    ...
                    pts = np.asarray(plt.ginput(n=-1, timeout=-1, show_clicks=True,
                                     mouse_stop=plt.MouseButton.RIGHT, mouse_pop=plt.MouseButton.MIDDLE))
                    print(pts)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--overwrite', default='False',
                        type=str2bool, help="Overwrite the existing label data.")
